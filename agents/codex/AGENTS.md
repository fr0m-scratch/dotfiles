...
- `Principal.md` is user-owned. `AOL.md` is append-only through
  `~/.codex/hooks/aol-append.sh`. Never add AI co-author/generated-by trailers.
- Use immutable, source-controlled skills and hook scripts. Keep sessions,
  auth, trust hashes, caches, databases, logs, plugin downloads, and mutable app
