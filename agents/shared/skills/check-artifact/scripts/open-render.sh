#!/usr/bin/env bash
# Portable file-preview dispatcher for the open-artifact and check-artifact skills.
...
case "$host" in
  ...
  otty)
    ...
    # session's pane explicitly. The caller pane id comes from $OTTY_OPEN_PANE (the agent sets it after
    ...
    # a background preview never yanks them off the active tab (set OTTY_OPEN_KEEP_FOCUS=1 to
    # stay on the preview instead). No caller id -> best-effort focused pane (legacy behavior).
    caller="${OTTY_OPEN_PANE:-${2:-}}"
    # Fall back to this agent session's cached pane id (written once by the caller;
    # keyed by the session id so concurrent sessions don't clobber each other).
    if [ -z "$caller" ]; then
      skey="${CODEX_SESSION_ID:-${CLAUDE_SESSION_ID:-}}"
