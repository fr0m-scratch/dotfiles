...
> 中文：这一节不讲技术细节，讲的是**我为什么这样用 Claude、为什么这样摆桌面**。
> 一句话总结：**fr0m 是一切的核心**——先立规矩，再干活；让纪律由工具自动守住，而不是靠记性。
...
| **AOL.md** | Append-only log — a timestamped trail of everything that was done. | The durable memory. Never edited, only appended, so the history can't be quietly rewritten. |
...
The full key map is in [`docs/keybindings.md`](docs/keybindings.md) and the
[`cheatsheet.html`](cheatsheet.html); the deeper "why/how" of each piece is in
[`docs/claude-code.md`](docs/claude-code.md) and [`docs/window-management.md`](docs/window-management.md).
