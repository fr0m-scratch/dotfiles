...
| | |
|---|---|
| 🧭 **How I actually work — the philosophy** | The *why* behind this setup: how I use Claude, why `fr0m` is the core, what the hooks and `/check` are for, and the keyboard-first desktop. Non-technical. → [`docs/how-i-work.md`](docs/how-i-work.md) |
| 🧭 **我是怎么用的 —— 设计哲学（中文）** | 同一篇说明的中文版：我怎么用 Claude、为什么 `fr0m` 是一切的核心、hook 与 `/check` 是干什么的、以及键盘优先的桌面哲学。 → [`docs/how-i-work.zh.md`](docs/how-i-work.zh.md) |
| 🗺️ **Shortcut & command cheatsheet (HTML)** | A printable, browser-openable reference card for every keybinding and command. Open it directly: → [`cheatsheet.html`](cheatsheet.html) |
